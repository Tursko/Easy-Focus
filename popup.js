...
function renderSessionTime() {
  ...
  popupSessionTime.textContent = sessionPaused
    ? `${elapsed} (Paused)`
    : elapsed;
}
...
async function getRestrictedSites() {
  ...
  try {
    let aParsed = JSON.parse(result.restrictedSites);
...
function renderUrlList() {
  Promise.all([
    getRestrictedSites(),
    chrome.storage.local.get("focusEnabled"),
  ]).then(([aRestrictedSites, result]) => {
    ulUrls.innerHTML = "";
    listDiv.style.display =
      aRestrictedSites.length > 0 && !result.focusEnabled ? show : hide;
    aRestrictedSites.forEach((url) => {
...
function togglePauseResume() {
  ...
  chrome.storage.local
    ...
    .then((result) => {
      ...
      let start = result.focusSessionStart
        ? new Date(result.focusSessionStart)
        : null;
      return chrome.storage.local.set({
        focusSessionAccumulatedMs: computeElapsedMs(
          result.focusSessionAccumulatedMs,
          start,
          false,
        ),
